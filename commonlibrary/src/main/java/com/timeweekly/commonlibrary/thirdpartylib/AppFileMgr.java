...
import com.timeweekly.commonlibrary.timeweeklylib.CommonLogger;
...
public class AppFileMgr {
    ...
    public static void deleteFilesByDirectory(File directory) {
        if (directory != null && directory.exists() && directory.isDirectory()) {
            ...
            CommonLogger.i("AppFileMgr-->>deleteFilesByDirectory", "This directory is not file, execute delete");
        }else{
        	CommonLogger.i("AppFileMgr-->>deleteFilesByDirectory", "This directory is file, not execute delete");
        }
    }
    ...
    public static long getFileSize(File file) {
    	...
    	CommonLogger.i("AppFileMgr-->>getFileSize", "This file size: " + size);
        return size;
    }
	...
	public static void saveFileToSdcard(Bitmap bitmap, String path){
		...
		if (!file.exists()) {
			...
			try {
				...
				outputStream.flush();
				outputStream.close();
				CommonLogger.i("AppFileMgr-->>saveFileToSdcard-->>bitmap:", bitmap.toString());
				CommonLogger.i("AppFileMgr-->>saveFileToSdcard-->>path:", path);
				CommonLogger.i("AppFileMgr-->>saveFileToSdcard:", "将File写入到指定路径下成功！");
			} catch (FileNotFoundException e) {
				e.printStackTrace();
				CommonLogger.e("AppFileMgr-->>saveFileToSdcard:", "将File写入到指定路径下失败！" + e.getMessage());
			} catch (IOException e1) {
				e1.printStackTrace();
				CommonLogger.e("AppFileMgr-->>saveFileToSdcard:", "将File写入到指定路径下失败！" + e1.getMessage());
			}
		}
	}
	...
	public static File getCacheFile(String imageUri) {
		...
		try {
			...
			e.printStackTrace();
			CommonLogger.e("AppFileMgr-->>getCacheFile:","获取Sdcard指定目录下缓存文件失败！" + e.getMessage());
		}
			CommonLogger.i("AppFileMgr-->>getCacheFile-->>imageUri:", imageUri);
			CommonLogger.i("AppFileMgr-->>getCacheFile:", "获取Sdcard指定目录下缓存文件成功！");
			return cacheFile;
	}
	...
	public static String getFileName(String path) {
		...
		CommonLogger.i("AppFileMgr-->>getFileName-->>path:", path);
	...
	public static void writeFileToSdCard(String fileContent, String fileName) {
		...
		if (sdCardFlag != null && sdCardFlag.equals(Environment.MEDIA_MOUNTED)) {
			CommonLogger.i("AppFileMgr-->>writeFileToSdCard-->>fileContent:",fileContent);
			CommonLogger.i("AppFileMgr-->>writeFileToSdCard-->>fileName:",fileName);
			...
			try {
				...
				e.printStackTrace();
				CommonLogger.e("AppFileMgr-->>writeFileToSdCard:","把内容写在SdCard卡上指定目录失败！" + e.getMessage());
			}
		} else {
				CommonLogger.e("AppFileMgr-->>writeFileToSdCard:","该SdCard不存在或不永许读写操作,写入失败！");
		}
	}
	...
	public File writeInputStreamToSdCard(String path, String fileName, InputStream inputStream){
		...
		try {
			CommonLogger.i("AppFileMgr-->>writeInputStreamToSdCard-->>path:",path);
			CommonLogger.i("AppFileMgr-->>writeInputStreamToSdCard-->>fileName:",fileName);
			CommonLogger.i("AppFileMgr-->>writeInputStreamToSdCard-->>inputStream:",inputStream + "");
			//创建目录和文件
			...
			e.printStackTrace();
			CommonLogger.e("AppFileMgr-->>writeInputStreamToSdCard:","将InputStream写入SdCard指定目录下失败！" + e.getMessage());
		} catch (IOException e) {
			e.printStackTrace();
			CommonLogger.e("AppFileMgr-->>writeInputStreamToSdCard:","将InputStream写入SdCard指定目录下失败！" + e.getMessage());
		}finally{
	...
	public static String readFileFromSdCard(String cacheFileName) {
		...
		try {
			...
			e.printStackTrace();
			CommonLogger.e("AppFileMgr-->>readFileFromSdCard:","从SdCard中读取文件内容失败！" + e.getMessage());
		}
			CommonLogger.i("AppFileMgr-->>readFileFromSdCard-->>cacheFileName:",cacheFileName);
			CommonLogger.i("AppFileMgr-->>readFileFromSdCard:","从SdCard中读取文件内容成功！");
	...
	public static String getRootFilePath(Context context, String path) {
		...
		if (Environment.getExternalStorageState().equals(Environment.MEDIA_MOUNTED)) {
			...
			CommonLogger.i("AppFileMgr-->>getRootFilePath","SdCard已存在, 在SdCard中创建文件夹！");
			CommonLogger.i("AppFileMgr-->>getRootFilePath-->>file", "创建文件夹路径为" + file);
		...
		//SdCard卡不存在
			...
			CommonLogger.i("AppFileMgr-->>getRootFilePath","SdCard卡不存在, 在手机中创建文件夹！");
			CommonLogger.i("AppFileMgr-->>getRootFilePath-->>file:","创建文件夹的路径为" + file);
			File files = new File(file);
			if (files == null || !files.exists()) {
				files.mkdir();
			}
	...
	public static File createSDFile(String filename){
			...
			try {
				...
				e.printStackTrace();
				CommonLogger.e("AppFileMgr-->>createSDFile:","创建文件失败！" + e.getMessage());
			}
				return file;
	}
	...
	public InputStream getInputStreamByStringURL(String urlStr){
		...
		try {
			...
			e.printStackTrace();
			CommonLogger.e("AppFileMgr-->>getInputStreamByStringURL:","根据地址获取InputStream失败！" + e.getMessage());
		} catch (IOException e){
			e.printStackTrace();
			CommonLogger.e("AppFileMgr-->>getInputStreamByStringURL:","根据地址获取InputStream失败！" + e.getMessage());
		}
    ...
    public static String getEncodeBase64File(String path) {
		...
		try {
		    ...
		    CommonLogger.i("AppFileMgr-->>getEncodeBase64File", "文件转换base64字符串成功！"  + strings);
		} catch (FileNotFoundException e) {
			e.printStackTrace();
			CommonLogger.e("AppFileMgr-->>getEncodeBase64File", "文件转换base64字符串失败！"  + e.getMessage());
		} catch(IOException e){
			e.printStackTrace();
			CommonLogger.e("AppFileMgr-->>getEncodeBase64File", "文件转换base64字符串失败！"  + e.getMessage());
		}
    ...
    public static void getDecoderBase64File(String base64Code, String targetPath) {
		...
		try {
	        ...
	        out.write(buffer);
	        out.close();
	        CommonLogger.i("AppFileMgr-->>getDecoderBase64File", "base64字符解码保存文件成功！");
		} catch (IOException e) {
			e.printStackTrace();
			CommonLogger.i("AppFileMgr-->>getDecoderBase64File", "base64字符解码保存文件失败！" + e.getMessage());
		}
    }
	...
	public static boolean cutFile(File src, File desc, boolean boolCover, boolean boolCut){	
		try {
			...
			//2：多文件剪切操作
				...
				CUT_FALG = false;
				e.printStackTrace();
				CommonLogger.e("AppFileMgr-->>cutFile:","文件剪切操作出现异常！" + e.getMessage());
		}	
				return CUT_FALG;
	}
	...
	public static boolean copyFile(File src, File desc){
		...
		if(src.isFile()){
				try {
					  ...
					  COPY_FALG = false;
					  e.printStackTrace();
					  CommonLogger.e("AppFileMgr-->>copyFile:","文件复制操作出现异常！" + e.getMessage());
				}finally{
						try {
							if(bis != null){ 	
								bis.close(); 	
							...
							COPY_FALG = false;
							e.printStackTrace();
							CommonLogger.e("AppFileMgr-->>copyFile:", "文件复制操作，数据流关闭出现异常！" + e.getMessage());
						}
				}
			}else if(src.isDirectory()){
				//创建目录
	...
	public static boolean renameFile(File file, String name){
		...
		CommonLogger.i("AppFileMgr-->>renameFile:", "文件重命名操作成功！");
	...
	public static boolean deleteFile(File file){
		try {
			...
			DELETE_FALG = false;
			e.printStackTrace();
			CommonLogger.i("AppFileMgr-->>deleteFile", "文件删除出现异常！" + e.getMessage());
		}
			return DELETE_FALG;
	}
